		...
		if instance_name.begins_with("KEY"):
			...
			if event.pressed:
				if is_black_key:
					...
					# Black keys are positioned at pos_x-7, pos_y+10 with size 14x50
					# White keys are positioned at pos_x, pos_y+50 with size 14x60
					# Overlap area: black key bottom is at pos_y+10+50 = pos_y+60
					# White key top is at pos_y+50, so overlap is from pos_y+50 to pos_y+60
					
					# Check if click is in the overlap area (y between pos_y+50 and pos_y+60)
					var pos_y = 455  # Base position from FrontCase.gd
					if local_pos.y >= pos_y + 50 and local_pos.y <= pos_y + 60:
